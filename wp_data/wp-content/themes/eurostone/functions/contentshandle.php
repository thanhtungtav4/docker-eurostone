...
function searchHandlerContent() {
	...
	if(is_array($paramsCategory)){
		$params['tax_query']= array(
			array(
					'taxonomy' => 'category-content',//double check your taxonomy name in you dd
					'field'    => 'id',
  ...
  if ( $dataPost->have_posts() ) :
    while ( $dataPost->have_posts() ) : $dataPost->the_post();
      $result  = array(
				...
				'images' => get_handle_thumbnail('CONTENT-ITEM-THUMB'),
				'date' => get_the_date('Y.m.d'),
				'category' =>get_contents_category(get_the_ID()),
			);
...
add_action('wp_ajax_searchHandlerContent', 'searchHandlerContent');
add_action('wp_ajax_nopriv_searchHandlerContent', 'searchHandlerContent');
